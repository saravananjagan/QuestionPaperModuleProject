...
class PartContainerComponent extends React.Component{
  ...
  AddPart(){
      ...
      this.partFormsId.push({pnameid:{pnameid}, pmarkid:{pmarkid}, pqid:{pqid}, pidmain:{pidmain}})
      this.PartForms=this.partFormsId.map((id)=>
        <PartComponent key={id.pqid.pqid} pnameid={id.pnameid.pnameid} pmarkid={id.pmarkid.pmarkid} pqid={id.pqid.pqid} pidmain={id.pidmain.pidmain}/>
      )
...
class PartComponent extends React.Component{
  ...
  AddQuestion(){
      ...
      this.QuestFormsId.push({qid:{qid}, pqcid:{pqcid}});
      ...
      this.QuestForms=this.QuestFormsId.map((questForm)=>
        <QuestionComponent key={questForm.pqcid.pqcid} qid={questForm.qid.qid} pqcid={questForm.pqcid.pqcid}/>
      )
      this.setState({
          quests:this.QuestForms
      });
...
class QuestionComponent extends React.Component{
  ...
  AddChoice(){
      ...
      this.ChoiceFormsId.push({cid:{cid}});
      this.ChoiceForms=this.ChoiceFormsId.map((id)=>
        <ChoiceInputComponent key={id.cid.cid} cid={id.cid.cid} />
      )
...
class Speech2TextModal extends React.Component{
  ...
  render(){
    return(
      <>
      <button type="button" className="btn btn-outline-success" style={{float:'right'}} onClick={this.openS2TModal}><i className="fa fa-microphone" aria-hidden="true"></i></button><br/>
      <Modal show={this.state.showModal}>
